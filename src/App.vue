...
const { render, cancel: cancelRender } = useMarkdown()
const tabs = ref<Tab[]>([])
...
function renderDocumentBytes(document: RenderDocument) {
...
function setTabDocument(tab: Tab, document: RenderDocument | null) {
  retainedResultBytes -= tab.renderedBytes
  tab.document = document
  tab.renderedBytes = document ? renderDocumentBytes(document) : 0
  retainedResultBytes += tab.renderedBytes
}
...
async function renderIntoTab(tab: Tab, payload: DocumentPayload, preserveError = false) {
  ...
  try {
    const document = await Promise.race([
      ...
      new Promise<RenderDocument>((_, reject) => {
        deadlineTimer = setTimeout(() => reject({ code: 'RENDER_TIMEOUT' }), RENDER_DEADLINE_MS)
      }),
    ])
    if (!tabs.value.some((item) => item.id === tab.id) || tab.renderGeneration !== generation) return
    setTabDocument(tab, document)
...
async function addDocument(payload: DocumentPayload) {
  const existing = tabs.value.find((tab) => tab.documentId === payload.documentId)
  if (existing) {
    if (!await activateTab(existing.id)) return
    const sourceChanged = existing.sourceContent !== payload.content
    setTabSource(existing, payload.content)
    touchTab(existing)
    if (sourceChanged || payload.sourceRevision > existing.sourceRevision) await renderIntoTab(existing, payload)
    else await ensureTabRendered(existing)
    return
  }
  const tab: Tab = {
  ...
  if (!await activateTab(reactiveTab.id)) return
  await renderIntoTab(reactiveTab, payload)
  try {
...
onMounted(async () => {
  ...
  await setupDragDrop()
  unlistenDocumentOpened = await listen<DocumentPayload>('document-opened', (event) => {
    void addDocument(event.payload)
  })
  ...
  try {
    const pending = await invoke<DocumentPayload | null>('take_pending_document')
    if (pending) await addDocument(pending)
  } catch {
    ...
    <header class="header-area">
      <div class="titlebar">
        ...
        <div class="titlebar-right">
          ...
          <button v-if="hasTabs" class="titlebar-btn icon-btn reader-settings-toggle" type="button" :title="t('readingSettings')" :aria-label="t('readingSettings')" @click="readerSettingsOpen = !readerSettingsOpen">
...
.titlebar-right { display: flex; align-items: center; gap: 4px; flex-shrink: 0; padding-bottom: 2px; }
...
.titlebar-btn.icon-btn { display: grid; width: 38px; min-height: 38px; place-items: center; padding: 0; font-size: 18px; }
.titlebar-btn.icon-btn svg { width: 18px; height: 18px; fill: none; stroke: currentColor; stroke-linecap: round; stroke-linejoin: round; stroke-width: 1.8; }
...
@media print {
  ...
  .header-area, .theme-status, .drag-overlay { display: none !important; }
}
</style>
